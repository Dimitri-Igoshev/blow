		...
		<div className="flex w-full flex-col px-3 sm:px-9 pt-[400px] xl:pt-[220px] gap-[30px] min-h-screen">
			<div className="hidden sm:flex w-full items-center justify-between">
				<h1 className="font-semibold text-[36px]">
					Результаты поиска <span className="text-primary text-[28px]">{search?.city ? cities?.find((city: any) => city.value === search.city)?.label : ""}</span>
				</h1>
			</div>
			{users?.length ? (
				<>
					...
					<div ref={ref} className="h-10 text-center text-gray-500"></div>
				</>
