  ...
  return (
    ...
    >
            ...
            <div className="col-span-12 sm:col-span-6 bg-white dark:bg-foreground-100 rounded-[36px] p-6 py-9 sm:p-[36px] flex flex-col justify-between">
              <div className="flex flex-col gap-6">
                <div className="flex justify-between items-start gap-6 owerflow-hidden w-full">
                  <div className="flex flex-col -mt-3">
                    ...
                    <p className="font-semibold line-clamp-1 text-[24px] xl:text-[36px]">
                      {user?.firstName ? user.firstName : user.sex === "male" ? "Мужчина" : "Девушка"}
                    </p>
                  ...
                  <div className="hidden lg:flex gap-1.5 items-center flex-nowrap min-w-[110px]">
                    {getActivityString(user?.activity) === "онлайн" ? (
                      ...
                      <p className="-mt-[2px] text-[12px]">{getActivityString(user?.activity)}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-1 pt-6">
                  ...
                  <p className="mt-1">
                    {user?.about && user.about !== 'undefined' ? maskContacts(user?.about, false) : "Пользователь предпочел не указывать информацию о себе."}
                  </p>
                </div>
              </div>
        ...
        <div>
          ...
          <div className="w-full h-full mt-20 flex justify-center px-6 sm:px-20">
            <p className="sm:text-[20px] text-center">
              Анкета была удалена пользователем или администрацией за нарушение правил платформы.
            </p>
          </div>
        </div>
      )}
      ...
      <InfoModal
        actionBtn="Купить"
        isOpen={isPremiumRequired}
        text={"Для того чтобы общаться с девушками, Вам нужно купить премиум подписку"}
        title={"Нужен премиум"}
      ...
      <InfoModal
        isOpen={isRegisterRequired}
        text={"Для того чтобы начать общаться, Вам нужно зарегистрироваться на сайте"}
      ...
      <InfoModal
        actionBtn="Купить"
        isOpen={isPremiumRequiredVoice}
        text={"Для того чтобы получить доступ к прослушиванию голоса, Вам нужно купить премиум подписку"}
        title={"Нужен премиум"}
        onAction={() => router.push(ROUTES.ACCOUNT.SERVICES)}
        onOpenChange={onPremiumRequiredVoiceChange}
      />
      ...
      <InfoModal
        isOpen={isRegistrationRequired}
        text={"Для того чтобы получить доступ к прослушиванию голоса, Вам нужна регистрация и премиум подписка"}
